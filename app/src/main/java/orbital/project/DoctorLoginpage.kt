...
class DoctorLoginpage : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        forgetpasswordd.setOnClickListener {
            startActivity(Intent(this,DoctorForgotPasswordActivity::class.java))
        }
        ...
        logind.setOnClickListener {
            when {
                TextUtils.isEmpty(loginusernamed.text.toString().trim { it <= ' ' }) -> {
                    Snackbar.make(
                        loginusernamed, "Please enter username",
                ...
                TextUtils.isEmpty(loginpasswordd.text.toString().trim { it <= ' ' }) -> {
                    Snackbar.make(
                        loginpasswordd, "Please enter password",
                        Snackbar.LENGTH_SHORT
                    ).show()
                }
                    ...
                    FirebaseAuth.getInstance().signInWithEmailAndPassword(username, password)
                        .addOnCompleteListener { task ->
                            if (task.isSuccessful) {
                                ...
                                Snackbar.make(
                                    loginusernamed, task.exception!!.message.toString(),
                                    Snackbar.LENGTH_SHORT
                                ).show()
                            }
                        }
